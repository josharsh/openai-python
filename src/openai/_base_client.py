...
class SyncAPIClient(BaseClient[httpx.Client, Stream[Any]]):
    ...
    def _sleep_for_retry(
        ...
        timeout = self._calculate_retry_timeout(max_retries - retries_taken, options, response.headers if response else None)
        log.info(
            "Retrying request to %s in %f seconds (retry %i of %i)",
            options.url,
            timeout,
            retries_taken + 1,
            max_retries,
